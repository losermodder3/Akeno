	...
	const mdata = await client.groupMetadata(anu.jid)
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Adeus senpai @${num.split('@')[0]} 馃嵀`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: ' s贸 em grupos! 鉂�',
					premium: ' ESTE PEDIDO 脡 SO PARA *USU脕RIOS PREMIUMS*',
					mod: 'ESTE PEDIDO 脡 ESPEC脥FICO PARA USUARIO MOD GREMORY*',
					benned: 'Voc锚 para a banda, por favor, contate o propriet谩rio para abrir sua banda',
					ownerG: 'n茫o.. n茫o somente o meu dono馃挏馃挏',
					ownerB: ' n茫o.. n茫o somente o meu dono馃挏馃挏',
					userB: `鈹�鈹�銆� LISTA 銆嶁攢鈹�\nOl谩 Kak !\nDesculpe, irm茫. Voc锚 n茫o est谩 registrado como amigo de rias. Registre-se para fazer amizade com o bot loser por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar loser|17\n\n鈹�鈹�銆� loser BOT 銆嶁攢鈹�`,
					admin: 'so adm! 鉂�',
					Badmin: 's贸 fa莽o se eu for adm馃檭馃挏',
				}
			}
			...
			const frendsowner = ["5521974747709@s.whatsapp.net","5511966491483@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5521973747709@s.whatsapp.net","15873162629@s.whatsapp.net","15877730070@s.whatsapp.net","5511966491483@s.whatsapp.net"] 
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau ot谩rio kapa kapa")
		}, 0)
	}
...
//@darkYT					
    ...
    break
				...
				case 'premiumlist':
					...
					teks += `鈹�+ Total : ${premium.length}\n鈺扳攢鈹�鈹�鈹�鈹�鈹�*銆� *loser BOT* 銆�*鈹�鈹�鈹�鈹�`
...
const bug = body.slice(10)
 if (args.length > 300) return client.sendMessage(from, 'M谩ximo 300 caracteres', msgType.text, {quoted: mek})
var nomor = mek.participant
...
break
                ...
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*GELAP BOS :V*')
				client.sendMessage(from, buffer, image, {quoted: mek, caption: 'lolii鉂も潳'})
				break
				 case 'hentai2':
				 data = fs.readFileSync('./src/hentai1.js');
                 jsonData = JSON.parse(data);
...
break
				...
				case 'openanime':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Modo anime on grupo 鉁旓笍')
					} else if (Number(args[0]) === 0) {
						anime.splice(from, 1)
						fs.writeFileSync('./src/anime.json', JSON.stringify(anime))
						reply('Modo anime Off no grupo 鉁旓笍')
					} else {
...
case 'antifake':
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`鈺攢鈹�鈹�鈹�*銆� *TOTAL DE USU脕RIOS loser BOT 馃憫* 銆峔n\`\`\``
					no = 0
					for (let hehehe of user) {
						no += 1
						teks += `\`\`\`[${no.toString()}]\`\`\` @${hehehe.split('@')[0]}\n`
					}
					teks += `鈹�+ Total de usu谩rios : ${user.length}\n鈺扳攢鈹�鈹�鈹�鈹�鈹�*銆� *loser* 銆�*鈹�鈹�鈹�鈹�`
...
// recursos anime, random @loser domina vida
          ...
          reply('Espere..')
			    ...
			    case 'owner':
                    ...
                    client.sendMessage(from, 'numero do meu amor, flooda o chat n茫o馃嵀',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								fs.unlinkSync(media)
								tipe = media.endsWith('.mp4') ? 'video' : 'gif'
								reply(`馃嵀 convers茫o falhou ${tipe} para o adesivo`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Voc锚 n茫o 茅 um Membro Premium, entre em contato com o loser ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					case 'dellvip':
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Voc锚 n茫o 茅 um Membro Premium, entre em contato com o propriet谩rio' ,text, { quoted: mek })
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*鈺攢鈹�鈹�鈹�*銆� *Loser鉁�* 銆峔n*鈹�+ wa.me/55973747709*鈺扳攢鈹�鈹�鈹�鈹�鈹�*銆� *Loser* 銆�*鈹�鈹�鈹�鈹�*\n\n*_SE QUER SER ADMIN DO BOT Akeno_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*鈺攢鈹�鈹�鈹�*銆� *PREMIUM USER馃憫* 銆峔n*鈹�+ wa.me/55973747709/*鈺扳攢鈹�鈹�鈹�鈹�鈹�*銆� *loser * 銆�*鈹�鈹�鈹�鈹�*\n\n*_SE QUER SER UM USU脕RIO PREMIUM DO loser akeno_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'animes':
					...
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTG_AmdkCJ9QaZpkQyQOu99YiaZojtQixB9NA&usqp=CAU`)
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'animezinhuuu^-^'})
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_suCkKowVEsDqAXsR0KBb-Lhr93JA2SyBvQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'animezinhuuu^-^'})
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQtvJ8LLVHeBRp3q_Oz7OqmQRcm3GdvFSK6lA&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'animezinhuuu^-^'})
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSkaiFL0adodsqlRpi816fVpM6v9aXL3hXNiQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'animezinhuuu^-^'})
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'mano馃槼 essa me deixou excitado'})
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'egirl pobre馃槼馃槼馃'})
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '鉂�'})
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ora ora bom dia senpai馃嵀鉂�'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde馃槑馃憤'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
				...
				case 'cjanao':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'agaragam馃槑馃馃锔�'})
					break
				case 'bolsoegirl':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT90iP0VMtZbCiwBITys-BkcYBcLoqZqgMjiw&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '锔廱onaro egirl馃槼馃槼'})
					break
				case 'malkova':
					memein = await kagApi.memeindo()
               ...
               if (args[0] === 'on') {
                  if (isAntiShit) return reply('antishit j谩 ativo burro');
                  antishit.push(from);
                  fs.writeFileSync('./database/json/antishit.json', JSON.stringify(antishit));
                  reply(`Antishit ativado`);
               } else if (args[0] === 'off') {
                  if (!isAntiShit) return reply('antishit j谩 ta off burro');
           ...
           case 'shitlist':
               ...
               break
				case 'rr':
                    rate = body.slice(1)
                    ratee = ["Tac... N茫o disparou","Tac... N茫o disparou,ainda...","Tac馃挜 Disparou e voc锚 morreu","Tac馃挜Disparou mas a bala pegou de rasp茫o","A arma falhou","Tac... Por pouco que n茫o dispara...","Tac... A arma estava descarregada"]
			...
			reply(mess.wait)
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('so meu dono馃挏')
        ...
        case 'promover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ok Ara Ara: @${mentioned[0].split('@')[0]} agora Voc锚 茅 adm!`, mentioned, true)
				...
				case 'admins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('{ 鉂� } *Marca a figurinha*')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.png')
                ...
                case 'bc3':
					...
					break
				case 'clearall':
					if (!isOwner) return reply('Quem 茅 Voc锚?')
					anu = await client.chats.all()
                ...
                reply('estou baixando senpai鉂ゐ煃�')
                query = args.join(' ')
...
case 'gerarnick': //@loserzinn 鎰�
teks = body.slice(10)
send = await fetchJson(`http://brizas-api.herokuapp.com/gerador/fancytext?apikey=brizaloka&text=${teks}`)
teks = ` 馃馃徎鈥嶁檪锔廚ICKS GERADOS COM SUCESSO!馃馃徎鈥嶁檪锔�
馃崣Primeiro ${send.random_1} 
馃崣Segundo ${send.random_2} 
馃崣T茅rceiro ${send.random_3} 
馃崣Quarto ${send.random_4} 
馃崣Quinto ${send.random_5}
 
   馃懢EXTRAS馃懢
 馃懢${send.squares}
 馃懢${send.inverted_squares}
 馃懢${send.italic}
 馃懢${send.bold}
 馃懢${send.future_alien}
 馃懢${send.asian_1}
 馃懢${send.asian_2}
 馃懢${send.squiggle}
 馃懢${send.squiggle_2}
...
鉃�    鈻夆晳鈻堚枑鈻夆枆鈻愨枑鈻嶁枅鈺戔枍鈻夆枏鈻嶁枍
鉃�    鈻夆晳鈻堚枑鈻夆枆鈻愨枑鈻嶁枅鈺戔枍鈻夆枏鈻嶁枍
    
    漏loserzinn 饾檷饾檧饾檻饾檴饾檱饾檺饾檹饾檮饾檴饾檳
 `
client.sendMessage(from, teks, text, {quoted: mek})
break	     	   			 		     	   			 	     	   			 		     	   			 
...
client.sendMessage(from, mp4, MessageType.video, {mimetype: 'video/gif', filename: `stick.gif`, quoted: mek, caption: '鉁�'})
...
if (args.length < 1) return reply(mess.blank)
teks = body.slice(10)
if (teks.length > 10) return reply('O texto 茅 longo, at茅 8 caracteres')
reply('*Estou fazendo... *')
...
//COMANDOS DE PESQUISAS
 case 'pucep':
 //@loserzinn 
 if (args.length < 1) return reply('Cad锟� O Cep')
cep = body.slice(6)
